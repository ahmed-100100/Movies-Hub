...
export default function StarDetails() {
    ...
    let type = window.location.pathname.includes("star")&&"person";
    let {id}=useParams();
    function getDetails(){
        axios
        ...
        <div className="row mt-3">
            <div className="col-md-4">
                {loading?
                    ...
                    (<img className="w-100" src={details.profile_path?imgPath(details.profile_path):starPlaceHolder} alt="" />)
                }
            </div>
            <div className="col-md-7 offset-1 d-flex align-items-center">
                <div>
                    <h2><span>Name: </span>{details?.name}</h2>
                    <h5><span>About: </span>{details?.biography}</h5>
                </div>
            </div>
        </div>
    </div>
