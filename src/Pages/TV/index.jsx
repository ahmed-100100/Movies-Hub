...
export default function TV() {
  let { tvs, tvPage, setTvPage } = useContext(MainContext);
  function next() {setTvPage((prevPage) => prevPage + 5);}
  function previous() {setTvPage((prevPage) => (prevPage > 5 ? prevPage - 5 : 1));}
  let startPage = Math.floor((tvPage - 1) / 5) * 5 + 1;
  let numbers = new Array(5).fill(1).map((_, i) => startPage + i);
  return (
    <div className="row my-3 justify-content-center">
      {tvs.length ? (
        tvs.map((tv) => <TvCard key={tv.id} tv={tv} />)
      ) : (
        <></>
      )}
        ...
        <ul className="pagination pagination-lg my-3 justify-content-center">
          ...
          {numbers.map((number) => (
            <li className="page-item" key={number}>
              <Link
                onClick={() => setTvPage(number)}
                className={`page-link ${number === tvPage ? 'bg-white' : 'bg-transparent'}`}
                to="#"
              >
                {number}
              </Link>
